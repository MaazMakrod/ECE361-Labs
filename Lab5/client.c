...
int directMessage(struct commandReturn *command);
int invite(struct commandReturn *command);
void *serverHandler();
...
int joinSession(struct commandReturn *command){
  ...
  // printf("Joining session %s", session_id);
...
int directMessage(struct commandReturn *command){
  ...
  char *user_id = strtok_r(NULL, " ", &(command->saveptr));
  char *dm_data = strtok_r(NULL, "", &(command->saveptr));

  free(command);

  if(user_id == NULL || dm_data == NULL){
    printf("Usage: /dm <user id> <message>\n");
    return INVALID;
  }
  // printf("send to user %s    data: %s\n", user_id, dm_data);

  char *dm_message = malloc(MAX_DATA);
  bzero(dm_message, MAX_DATA);
  strcpy(dm_message, user_id);
  strcat(dm_message, "^");
  strcat(dm_message, client.name);
  strcat(dm_message, " (dm): ");
  strcat(dm_message, dm_data);

  struct message *m = (struct message *) malloc(sizeof(struct message));
  bzero(m->source, MAX_NAME);
  bzero(m->data, MAX_DATA);
  m->type = DM;
  m->size = strlen(dm_message);
  memcpy(&(m->source), client.name, strlen(client.name));
  memcpy(&(m->data), dm_message, strlen(dm_message));
...
int invite(struct commandReturn *command){
  ...
  char *invite_type = strtok_r(NULL, " ", &(command->saveptr));
  char *invite_command = strtok_r(NULL, "", &(command->saveptr));

  if(invite_type == NULL || invite_command == NULL || (strcmp(invite_type, "response") != 0 && strcmp(invite_type, "send") != 0)){
    printf("Usage: /invite <response | send> <accept | reject | user id>\n");
    return INVALID;
  }

  if(strcmp(invite_type, "response") == 0 && strcmp(invite_command, "accept") != 0 && strcmp(invite_command, "reject") != 0){
    printf("A response to an invite must be followed by accept or reject\n");
    return INVALID;
  }

  if(strcmp(invite_type, "response") == 0){

    if(invitedSession == NULL){
      printf("Error: you do not currently have an invitation to accept or reject\n");
      return INVALID;
    }

    if(strcmp(invite_command, "accept") == 0){
      char *session = malloc(MAX_DATA);
      bzero(session, MAX_DATA);
      strcat(session, "/joinsession ");
      strcat(session, invitedSession);

      // printf("%s | %s\n", invitedSession, session);

      struct commandReturn *c = (struct commandReturn *) malloc(sizeof(struct commandReturn));
      c->type = JOIN;

      strtok_r(session, " ", &c->saveptr);
      joinSession(c);
      free(session);
    } else {
      printf("You have rejected the invitation to join %s\n", invitedSession);
    }
    ...
    if(client.currentSession == NULL){
      printf("Error you must be in a session to invite a user\n");
      return INVALID;
    }

    char *invite_message = malloc(MAX_DATA);
    bzero(invite_message, MAX_DATA);

    strcpy(invite_message, client.currentSession);
    strcat(invite_message, "^");
    strcat(invite_message, invite_command);
    strcat(invite_message, "^");
    strcat(invite_message, client.name);
    strcat(invite_message, " has invited you to join ");
    strcat(invite_message, client.currentSession);
    strcat(invite_message, ". Respond using /invite response <accept | reject>");
    ...
    bzero(m->data, MAX_DATA);
    m->type = INVITE;
    m->size = strlen(invite_message);
    memcpy(&(m->source), client.name, strlen(client.name));
    memcpy(&(m->data), invite_message, strlen(invite_message));
...
void *serverHandler(){
  ...
  while(1){
    ...
    if(m->type == JN_ACK){
      ...
      pthread_mutex_lock(&client_lock);
      invitedSession = strdup(session_id);
      pthread_mutex_unlock(&client_lock);
    } else{
